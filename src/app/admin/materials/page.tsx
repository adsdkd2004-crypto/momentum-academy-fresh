...
export default function AdminMaterialsPage() {
  ...
  const [classId, setClassId] =
    useState<"class-9" | "class-10">("class-10");
  const [subject, setSubject] =
    useState<SubjectName>("Mathematics");
  const [chapterTitle, setChapterTitle] = useState("");
  const [type, setType] =
    useState<MaterialType>("Handwritten Notes");
  ...
  const handleUpload = async () => {
    if (!file || !chapterTitle.trim() || !user) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "Could not prepare the upload."
        );
      }
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <select
            ...
            onChange={(e) =>
              setClassId(
                e.target.value as "class-9" | "class-10"
              )
            }
        ...
        {progress !== null && (
          <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-navy-100 dark:bg-white/10">
            <div
              className="h-full bg-momentum-500 transition-all"
      ...
      <div className="card p-6">
        ...
        <div className="mt-4 space-y-2">
          {materials.map((m) => (
            <div
              key={m.id}
              className="flex items-center justify-between rounded-lg border border-navy-100 p-3 text-sm dark:border-white/10"
            >
              <div className="min-w-0">
                <p className="truncate font-medium text-navy-700 dark:text-paper">
                  {m.fileName}
                </p>
